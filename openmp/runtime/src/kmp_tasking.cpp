...
//To manage replica's allocated data
struct ReplicaAllocatedData *replicasAllocatedData;
int numAllocatedReplicasDataSize = 0;
int numAllocatedReplicas = 0;
...
//Lock for managing replica's data allocation
kmp_futex_lock_t ReplicasDataLock =
    KMP_FUTEX_LOCK_INITIALIZER(ReplicasDataLock);
...
void *__kmpc_set_replicate_variable(void *original, kmp_int64 pragmaID,
  ...
  if (!numAllocatedReplicasDataSize) {
    replicasAllocatedData = (struct ReplicaAllocatedData *)malloc(
        5 * sizeof(struct ReplicaAllocatedData));
    for (int i = 0; i < 5; i++) {
      replicasAllocatedData[i].pragmaID = -1;
      replicasAllocatedData[i].occupied = 0;
    }
    numAllocatedReplicasDataSize = 5;
  } else if (numAllocatedReplicasDataSize == numAllocatedReplicas) {
    replicasAllocatedData = (struct ReplicaAllocatedData *)realloc(
        replicasAllocatedData,
        numAllocatedReplicasDataSize * 2 * sizeof(struct ReplicaAllocatedData));
    for (int i = numAllocatedReplicasDataSize;
         i < numAllocatedReplicasDataSize * 2; i++) {
      replicasAllocatedData[i].pragmaID = -1;
      replicasAllocatedData[i].occupied = 0;
    }
    numAllocatedReplicasDataSize *= 2;
  }
  ...
  for (int i = 0; i < numAllocatedReplicasDataSize; i++) {
    if (replicasAllocatedData[i].pragmaID == -1) {
      ...
      replicasAllocatedData[i].data = newAllocatedData;
      numAllocatedReplicas++;
      __kmp_release_futex_lock(&ReplicasDataLock, 0);
      return newAllocatedData;
    }
  }
  return nullptr;
}
