...
void CodeGenFunction::EmitOMPTaskDirective(const OMPTaskDirective &S) {
  ...
  SmallVector<std::pair <StringRef, StringRef>> VariantRefs;
  if (NumReplicas) {
  ...
  auto &&TaskGenOriginal = [&S, SharedsTy, CapturedStruct,
  ...
  // When having variants, count original as replica already
  //if(NumVariants) NumReplicas = NumReplicas - 1;
  ...
  if (NumReplicas) {
    ...
    for (int i = 0; i < NumReplicas-1; i++) {
      ...
      auto &&TaskGen = [&S, SharedsTy, IfCond, CapturedStructCopy, &VariantRefs,
                        ...
                        this](CodeGenFunction &CGF, llvm::Function *OutlinedFn,
                              const OMPTaskDataTy &Data) {
        CGF.CGM.getOpenMPRuntime().emitTaskCall(
            CGF, S.getBeginLoc(), S, OutlinedFn, SharedsTy, CapturedStructCopy,
            IfCond, Data);
        ...
        for (llvm::BasicBlock &bb : *OutlinedFn) {
          for (llvm::Instruction &I : bb) {
            if (llvm::CallBase *call = dyn_cast<llvm::CallBase>(&I)) {
              ...
              SmallVector<llvm::Function *, 8> VariantFunctions;
              llvm::Function *CalledFunction = call->getCalledFunction();
              if (CalledFunction && !CalledFunction->isDeclaration() &&
                  CalledFunction->hasFnAttribute("variant")) {

                StringRef MainVariantName =
                    CalledFunction->getFnAttribute("variant")
                        .getValueAsString();

                llvm::LLVMContext &C = I.getContext();
                llvm::MDNode *N = llvm::MDNode::get(
                    C, llvm::MDString::get(C, std::to_string(i + 1)));
                I.setMetadata("task.replica.id", N);

                // Process Variants Statically: Commented since we want variants
                ...
                llvm::GlobalVariable *FunctionsArray =
                    CGM.getModule().getGlobalVariable(VariantsName, true);
                llvm::ConstantArray *FunctionsConstantArray =
                    dyn_cast<llvm::ConstantArray>(
                        FunctionsArray->getOperand(0));
                int numVariants = FunctionsConstantArray->getNumOperands();

                for (int i = 0; i < numVariants; i++) {
                  VariantFunctions.push_back(dyn_cast<llvm::Function>(
                      FunctionsConstantArray->getOperand(i)));
                }*/
              }
              if (VariantFunctions.size()) {
                auto it = VariantReplicaCount.find(call->getCalledFunction());
                int VarNumber = 1;
                if (it == VariantReplicaCount.end()) {
                  // not found
                  VariantReplicaCount[CalledFunction] = VarNumber;
                } else {
                  VarNumber = it->second + 1;
                  VariantReplicaCount[CalledFunction] = VarNumber;
                  // found
                }
                if (VarNumber <= (int) VariantFunctions.size())
                  ...
                  llvm::dbgs()
                      << "Function has more replicas than variants! \n";
              }
            }
          }
        }
