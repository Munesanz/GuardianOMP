...
// Function to emit two local arrays from ArrayRef<std::pair<Expr *, Expr *>>
std::pair<llvm::Value*, llvm::Value*> emitLocalArrays(SmallVector<std::pair<llvm::Value *, llvm::Value *>> VarFunc, CodeGenModule &CGM, CodeGenFunction &CGF) {
    // Create arrays for first and second elements
    size_t size = VarFunc.size();
    if(!size){
      return std::make_pair(llvm::ConstantPointerNull::get(CGM.VoidPtrTy), llvm::ConstantPointerNull::get(CGM.VoidPtrTy));
    }
    llvm::ArrayType *ArrayTy = llvm::ArrayType::get(CGM.Int8PtrTy, size); // Assuming Expr * is represented as i8* (pointer to int8)

    // Allocate local arrays on the stack
    llvm::AllocaInst *FirstArray = CGF.Builder.CreateAlloca(ArrayTy, nullptr, "vars");
    llvm::AllocaInst *SecondArray = CGF.Builder.CreateAlloca(ArrayTy, nullptr, "functions");

    // Store values into the arrays
    for (size_t i = 0; i < size; ++i) {
        // Get the elements
        llvm::Value *firstElem = VarFunc[i].first;
        llvm::Value *secondElem = VarFunc[i].second;

        llvm::Value *FirstArrayElemPtr = CGF.Builder.CreateInBoundsGEP(ArrayTy, FirstArray, {llvm::ConstantInt::get(CGM.Int32Ty, 0),llvm::ConstantInt::get(CGM.Int32Ty, i)});
        llvm::Value *SecondArrayElemPtr = CGF.Builder.CreateInBoundsGEP(ArrayTy, SecondArray, {llvm::ConstantInt::get(CGM.Int32Ty, 0),llvm::ConstantInt::get(CGM.Int32Ty, i)});
        unsigned Alignment = CGF.getContext().getTypeAlignInChars(CGF.getContext().VoidPtrTy).getQuantity();
        // Store the elements into the arrays
        CGF.Builder.CreateStore(firstElem, Address(FirstArrayElemPtr, CGM.VoidPtrTy, CharUnits::fromQuantity(Alignment)));
        CGF.Builder.CreateStore(secondElem, Address(SecondArrayElemPtr, CGM.VoidPtrTy, CharUnits::fromQuantity(Alignment)));
    }
    return std::make_pair(FirstArray, SecondArray);
...
void CGOpenMPRuntime::emitTaskCall(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  auto &&ThenCodeGen = [this, &Data, TDBase, KmpTaskTQTyRD, &TaskArgs,
                        &DepTaskArgs, NewTask, &D, ThreadID, Loc,
                        TaskFunction](CodeGenFunction &CGF, PrePostActionTy &) {
    if (!Data.Tied) {
      auto PartIdFI = std::next(KmpTaskTQTyRD->field_begin(), KmpTaskTPartId);
      LValue PartIdLVal = CGF.EmitLValueForField(TDBase, *PartIdFI);
      CGF.EmitStoreOfScalar(CGF.Builder.getInt32(0), PartIdLVal);
    }
    // Try to get the allocated var for the replica generated by CapturedStmt,
    // if it is not present means that the variable is either a global variable
    // or not used in the task.
    SmallVector<llvm::Value *> ReplicaAllocatedVars = Data.AllocatedVars;
    llvm::SmallVector<std::pair<Expr *, int>> Vars = Data.ProcessedVars;
    llvm::SmallVector<std::pair<Expr *, Expr *>> VarsDeepSizes = Data.ProcessedVarsDeepSizes;

    if (D.getSingleClause<OMPReplicatedClause>() && Data.IsReplica &&
        !Data.IsLastReplicaNode) {
      // If not present, allocate a new var for the replica now
      for (int i = 0; i < (int)Vars.size(); i++) {
        if(ReplicaAllocatedVars[i]!=nullptr)
          continue;
        ...
        if (ArraySizeExpr) {
          ArraySizeValue = CGF.EmitAnyExpr(ArraySizeExpr).getScalarVal();
          ArraySizeValue =
              CGF.Builder.CreateZExt(ArraySizeValue, CGF.CGM.Int64Ty);
        } else {
          ArraySizeValue = llvm::ConstantInt::get(CGF.CGM.Int64Ty, 0);
        }

        ArraySizeExpr = VarsDeepSizes[i].second;
        if (ArraySizeExpr) {
          SecondArraySizeValue = CGF.EmitAnyExpr(ArraySizeExpr).getScalarVal();
          SecondArraySizeValue =
              CGF.Builder.CreateZExt(ArraySizeValue, CGF.CGM.Int64Ty);
        } else {
          SecondArraySizeValue = llvm::ConstantInt::get(CGF.CGM.Int64Ty, 0);
        }

        // Obtain original variable info
        Expr *VarToReplicate = Vars[i].first;
        LValue OriginalLValue = CGF.EmitLValue(VarToReplicate);
        llvm::Value *OriginalVarPointer = OriginalLValue.getPointer(CGF);
        llvm::Value *OriginalVarValue =
            CGF.Builder.CreateLoad(OriginalLValue.getAddress(CGF));

        // Ask the runtime to generate the replicate variable
        const llvm::DataLayout &DataLayout = CGM.getModule().getDataLayout();
        llvm::Value *ReplicateArgs[] = {
            OriginalVarPointer,
            llvm::ConstantInt::get(CGF.CGM.Int64Ty, Data.PragmaID),
            llvm::ConstantInt::get(CGF.CGM.Int32Ty, Data.ReplicaID),
            llvm::ConstantInt::get(
                CGM.Int32Ty,
                DataLayout.getTypeAllocSize(OriginalVarValue->getType())),
            ArraySizeValue,
            SecondArraySizeValue,
            llvm::ConstantInt::get(CGF.CGM.Int32Ty, i),
            llvm::ConstantInt::get(CGF.CGM.Int32Ty, 1)};

        llvm::Value *ReplicaAllocatedVar = CGF.EmitRuntimeCall(
            OMPBuilder.getOrCreateRuntimeFunction(
                CGM.getModule(), OMPRTL___kmpc_set_replicate_variable),
            ReplicateArgs);
        ReplicaAllocatedVars[i]= ReplicaAllocatedVar;
        // Get the replicated variable inside the task
        auto OldIP = CGF.Builder.saveIP();
        CGF.Builder.SetInsertPoint(TaskFunction->front().getFirstNonPHI());
        llvm::Value *getReplicateArgs[] = {
            llvm::ConstantInt::get(CGF.CGM.Int64Ty, Data.PragmaID),
            llvm::ConstantInt::get(CGF.CGM.Int32Ty, Data.ReplicaID),
            llvm::ConstantInt::get(CGF.CGM.Int32Ty, i)};

        llvm::Value *getAllocatedVariable = CGF.EmitRuntimeCall(
            OMPBuilder.getOrCreateRuntimeFunction(
                CGM.getModule(), OMPRTL___kmpc_get_replicate_variable),
            getReplicateArgs);
        auto TaskIP = CGF.Builder.saveIP();
        ...
        OriginalVarPointer->replaceUsesWithIf(
            getAllocatedVariable, [TaskFunction](llvm::Use &U) {
              llvm::Instruction *I = dyn_cast<llvm::Instruction>(U.getUser());
              return I && I->getFunction() == TaskFunction;
            });
        ...
        for (llvm::User *GVarUser : OriginalVarPointer->users()) {
          if (llvm::GEPOperator *IUser =
            ...
            if (IUser->getPointerOperand() == OriginalVarPointer) {
              ...
              CGF.Builder.restoreIP(TaskIP);
              llvm::Value *NewGEP = CGF.Builder.CreateInBoundsGEP(
                  OriginalVarValue->getType(), getAllocatedVariable, Indices);
              CGF.Builder.restoreIP(OldIP);

              IUser->replaceUsesWithIf(NewGEP, [TaskFunction](llvm::Use &U) {
                llvm::Instruction *I = dyn_cast<llvm::Instruction>(U.getUser());
                return I && I->getFunction() == TaskFunction;
              });
            }
    ...
    if (D.getSingleClause<OMPReplicatedClause>() && !Data.IsLastReplicaNode) {
      const auto *replicatedClause =
              D.getSingleClause<OMPReplicatedClause>();
      ArrayRef<std::pair<Expr *, Expr *>> VarFunc;
      if (const auto *ConsolidationClause =
              D.getSingleClause<OMPConsolidationClause>()) {
        VarFunc = ConsolidationClause->getVarFunc();
      }
      SmallVector<std::pair<llvm::Value *, llvm::Value*>> VarsToCheck;
      for (int i = 0; i < (int)VarFunc.size(); i++) {
        int found = 0;
        int j;

        for (j = 0; j < (int)Vars.size(); j++) {
          // Ensure both Expr instances are DeclRefExpr
          const DeclRefExpr *DRE1 =
              dyn_cast<DeclRefExpr>(Vars[j].first->IgnoreParenCasts());
          const DeclRefExpr *DRE2 =
              dyn_cast<DeclRefExpr>(VarFunc[i].first->IgnoreParenCasts());
        ...
        if (!found)
          continue;
        llvm::Value *CallbackPointer =
            llvm::ConstantPointerNull::get(CGF.CGM.VoidPtrTy);
        if (VarFunc[i].second)
          CallbackPointer = CGF.EmitLValue(VarFunc[i].second).getPointer(CGF);
        // Original task does not need to look for the replicated var
        if (!Data.IsReplica) {
          Expr *VarToReplicate = Vars[j].first;
          llvm::Value *OriginalVarPointer =
              CGF.EmitLValue(VarToReplicate).getPointer(CGF);

          VarsToCheck.push_back({OriginalVarPointer, CallbackPointer});
        } else {
          VarsToCheck.push_back({ReplicaAllocatedVars[j], CallbackPointer});
        }
      }

      std::pair<llvm::Value *, llvm::Value *> Arrays = emitLocalArrays(VarsToCheck, CGF.CGM, CGF);

      // Generate kmpc_prepare_taskwait call
      llvm::Value *ConstraintValue = nullptr;
      OpenMPReplicatedKeyword constraint =
          replicatedClause->getReplicatedKeyword();
      if (constraint == OMPC_REPLICATED_KEYWORD_spatial ||
          constraint == OMPC_REPLICATED_KEYWORD_spatial_temporal) {
        ConstraintValue = CGF.Builder.getInt1(1);
      } else {
        ConstraintValue = CGF.Builder.getInt1(0);
      }

      llvm::Value *PrepareTaskwaitArgs[] = {
          NewTask, Arrays.first, Arrays.second, CGF.Builder.getInt32(VarsToCheck.size()), Data.GroupID, ThreadID, ConstraintValue};
