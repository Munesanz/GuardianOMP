...
void CGOpenMPRuntime::emitTaskCall(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  auto &&ThenCodeGen = [this, &Data, TDBase, KmpTaskTQTyRD, &TaskArgs,
    ...
    //Try to get the allocated var for the replica generated by CapturedStmt, if it is not present means that the variable is either a global variable or not used in the task.
    llvm::Value *ReplicaAllocatedVar = nullptr;
    SmallVector<llvm::Value *, 4> AllocatedVars = Data.AllocatedVars;
    for (int i = 0; i < (int) AllocatedVars.size(); i++) {
      ReplicaAllocatedVar = AllocatedVars[i];
      if (D.getSingleClause<OMPReplicatedClause>() && Data.IsReplica &&
          !Data.IsLastReplicaNode && ReplicaAllocatedVar == nullptr) {
        ...
        if(i== (int) AllocatedVars.size()-1)
         ArraySizeExpr = replicatedClause->getArraySize();
        else
         ArraySizeExpr = replicatedClause->getCopyInArraySizes()[i].first;
        ...
        Expr *SecondSizeExpr;
        if(i== (int) AllocatedVars.size()-1)
          SecondSizeExpr = nullptr;
        else
          SecondSizeExpr =  replicatedClause->getCopyInArraySizes()[i].second;
        llvm::Value *SecondSizeValue = nullptr;
        if (SecondSizeExpr) {
          SecondSizeValue = CGF.EmitAnyExpr(SecondSizeExpr).getScalarVal();
          SecondSizeValue = CGF.Builder.CreateZExt(SecondSizeValue, CGF.CGM.Int64Ty);
        } else {
          SecondSizeValue = llvm::ConstantInt::get(CGF.CGM.Int64Ty, 0);
        }
        ...
        if (i == (int)AllocatedVars.size() - 1)
          ...
          VarToReplicate = replicatedClause->getCopyInVars()[i];
        ...
        ReplicaAllocatedVar = CGF.EmitRuntimeCall(
            OMPBuilder.getOrCreateRuntimeFunction(
    ...
    if (D.getSingleClause<OMPReplicatedClause>() && !Data.IsLastReplicaNode) {
      ...
      llvm::ConstantInt *MainVariableSize = CGF.Builder.getInt32(0);
      if (!Data.IsReplica) {
        Expr *VarToReplicate =
        ...
        const llvm::DataLayout &DataLayout = CGM.getModule().getDataLayout();
        MainVariableSize =  llvm::ConstantInt::get(
                CGM.Int32Ty,
                DataLayout.getTypeAllocSize(OriginalVarValue->getType()));
      } else {
        // Replicas are generated as global variables
        ReplicatedVar = ReplicaAllocatedVar;
      }
      if (!ReplicatedVar) {
        unsigned DiagID = CGM.getDiags().getCustomDiagID(
            DiagnosticsEngine::Error, "Original or replicated variables "
                                      "for tasks and replicas are not "
                                      "found when creating "
      ...
      OpenMPRedundancyConstraint constraint =
          replicatedClause->getRedundancyConstraint();
      if (constraint == OMPC_REDUNDANCY_CONSTRAINT_spatial ||
          constraint == OMPC_REDUNDANCY_CONSTRAINT_spatial_temporal) {
      ...
      llvm::Value *PrepareTaskwaitArgs[] = {
          NewTask, ReplicatedVar, MainVariableSize, Data.GroupID, ThreadID, ConstraintValue};
